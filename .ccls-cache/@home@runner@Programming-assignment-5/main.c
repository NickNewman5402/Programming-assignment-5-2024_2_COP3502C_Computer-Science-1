...
tree_node* delete(tree_node* root, int value);
...
int main(void) {
  ...
  int numInputs, points;
  char command[14], name[25];
  ...
  for (int i = 0; i < numInputs; i++) {
    ...
    if(strcmp(command, "height_balance") == 0){
      int a = numNodesLeft(my_root);
      int b = numNodesRight(my_root);
